<template>
    <div>
        <div class="bg-red-lightest border-t-4 border-red rounded-b text-red-darkest px-4 py-3 shadow-md mb-2" role="alert" v-if="error != null">
            <div class="flex">
                <div class="py-1">
                    <svg class="fill-current h-6 w-6 text-red mr-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                        <path d="M2.93 17.07A10 10 0 1 1 17.07 2.93 10 10 0 0 1 2.93 17.07zm12.73-1.41A8 8 0 1 0 4.34 4.34a8 8 0 0 0 11.32 11.32zM9 11V9h2v6H9v-4zm0-6h2v2H9V5z"/>
                    </svg>
                </div>
                <div>
                    {{ error }}
                </div>
            </div>
        </div>
        <ul>
            <li v-for="site in sites" :key="site.id">
                {{ site.name }}
            </li>
        </ul>
    </div>
</template>

<script>
    import ForgeApi from './../apis/forge_api';

    export default {
        name: "select-syncable-sites",
        data() {
            return {
                sites: [],
                error: null
            }
        },
        mounted() {
            this.getSites();
        },
        methods: {
            getSites() {
                ForgeApi.servers().then((response) => {
                    response.data.servers.forEach(function (value, key) {
                        ForgeApi.sites(value.id).then((response) => {
                            response.data.sites.forEach(function (site, skey) {
                                this.sites.push({name: site.name, site: id});
                            });
                        });
                    });
                });
            }
        }
    }
</script>

<style scoped>

</style>